...
{
    ui->setupUi(this);
    connect(ui->apply_friend_listWidget, &ApplyFriendList::sig_show_search, this, &ApplyFriendPage::sig_show_search);
    loadApplyList();
    //接受tcp传递的authrsp信号处理
...
void ApplyFriendPage::AddNewApply(std::shared_ptr<AddFriendApply> apply)
{
    ...
    ui->apply_friend_listWidget->insertItem(0,item);
    ui->apply_friend_listWidget->setItemWidget(item, apply_item);
...
void ApplyFriendPage::loadApplyList()
{
    ...
    for(int i = 0; i < 13; i++){
        ...
        item->setSizeHint(apply_item->sizeHint());
        item->setFlags(item->flags() & ~Qt::ItemIsEnabled & ~Qt::ItemIsSelectable);
        ui->apply_friend_listWidget->addItem(item);
        ui->apply_friend_listWidget->setItemWidget(item, apply_item);
        //收到审核好友信号
        connect(apply_item, &ApplyFriendItem::sig_auth_friend, [this](std::shared_ptr<ApplyInfo> apply_info){
            //            auto *authFriend =  new AuthenFriend(this);
